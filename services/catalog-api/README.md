# BIT DiDaCa Catalog API

This independently runnable FastAPI project is the metadata store and Policy Administration Point (PAP) for one standalone DaCa catalog instance. For this POC it owns a local SQLite database; a later PostgreSQL migration can keep the existing SQLAlchemy and Alembic boundary. The service contains aggregate-only synthetic ESTV examples and no real taxpayer or personal data.
...
$env:DATABASE_URL = "sqlite+pysqlite:///./didaca-catalog.db"
uv run --no-workspace alembic upgrade head
uv run --no-workspace didaca-catalog-seed
uv run --no-workspace uvicorn didaca_catalog.main:app --reload --port 8001
```

From the monorepo, use the root Compose workflow. The container stores `didaca-catalog.db` on the
named `didaca-catalog-sqlite` volume, then runs Alembic and the idempotent deterministic seed before
Uvicorn. The service listens on port `8001`; OpenAPI is at `/docs`.

Configuration is read from environment variables. Copy `.env.example` for local use and replace its placeholder credentials. `SAMPLE_POLICY_PROJECTION_URL` is the base path ending in `policy-projections`; the product UUID is appended. A `{product_id}` placeholder is also accepted. The bearer token can be supplied as either `SAMPLE_POLICY_PROJECTION_TOKEN` or `DIDACA_POLICY_DEPLOYMENT_TOKEN`.
...
curl -i -X PATCH \
  ...
  -H 'If-Match: "1"' \
  -H 'X-DiDaCa-User: estv-data-owner' \
...
2. Publishing adds an immutable revision. OPA polls the bundle route and atomically loads `data.json` plus generated package `didaca.authz`.
...
4. The HTTP PEP asks OPA for `data.didaca.authz.decision`. The sample product fails closed if that decision is unavailable or malformed.
...
The focused suite uses SQLite for deterministic API contract tests. PostgreSQL-specific ACL/RLS enforcement is tested by the sibling sample-data-product project and by the root Compose integration workflow.
