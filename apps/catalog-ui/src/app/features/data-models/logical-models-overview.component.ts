...
@Component({
  ...
  template: `
    ...
    <div class="models-actions">
      <a class="daca-button" routerLink="/models/new" [class.is-disabled]="!identity.canEditModels()" [attr.aria-disabled]="!identity.canEditModels()">Neues logisches Modell</a>
      <a class="daca-button is-secondary" routerLink="/physical-models">Physische Modelle</a>
    ...
    @if (loading()) {
      ...
      <div class="daca-card models-state"><h2>Keine passenden Modelle</h2><p>{{ models().length ? 'Passen Sie die Filter an.' : 'Erstellen Sie das erste unabhängige logische Modell.' }}</p><a class="daca-button" routerLink="/models/new">Neues logisches Modell</a></div>
    }
  `,
  styles: [`
...
export class LogicalModelsOverviewComponent {
  ...
  dateLabel(value: string): string { return value ? new Intl.DateTimeFormat('de-CH', { dateStyle: 'medium' }).format(new Date(value)) : '–'; }
