  ...
  template: `
    ...
    @if (!loading()) {
      <form [formGroup]="form" (ngSubmit)="save()" class="model-editor-form">
            ...
            @for (field of fields.controls; track field; let index = $index) {
              <fieldset [formGroupName]="index"><legend>Feld {{ index + 1 }} · {{ field.controls.name.value || 'Unbenannt' }}</legend><button class="remove-field" type="button" [disabled]="fields.length === 1" (click)="removeField(index)">Feld entfernen</button>
                <div class="editor-grid">
                  ...
                  <label class="is-wide">I14Y-Concept-Links<select class="concept-multi" multiple size="5" formControlName="conceptIds" (change)="selectConcepts(index, selectValues($event))">@for(concept of concepts();track concept.id){<option [value]="concept.id" [attr.data-concept-id]="concept.id">{{ conceptName(concept) }} · {{ concept.conceptType }} · v{{ concept.version }}</option>}@for(conceptId of missingConceptIds(field);track conceptId){<option [value]="conceptId" [attr.data-concept-id]="conceptId">{{ conceptLabel(conceptId) }}</option>}</select><small>Mehrere Concepts können mit Strg/⌘ oder Umschalt ausgewählt werden.</small></label>
                  <label class="is-wide">Primär für I14Y<select formControlName="primaryConceptId" [required]="linkedConceptIds(field).length > 0">@if(!linkedConceptIds(field).length){<option value="">Kein Concept verknüpft</option>}@for(conceptId of linkedConceptIds(field);track conceptId){<option [value]="conceptId">{{ conceptLabel(conceptId) }}</option>}</select><small>Genau dieses Concept wird als <code>dcterms:conformsTo</code> exportiert.</small></label>
                  <label class="is-wide">Werteliste (I14Y CodeList)<select formControlName="valueListConceptId" (change)="selectConcept(index, selectValue($event), true)"><option value="">Keine Werteliste</option>@for(concept of codeLists();track concept.id){<option [value]="concept.id">{{ conceptName(concept) }} · {{ concept.identifiers[0] }}</option>}</select></label>
                </div>
              </fieldset>
            }
          </div>
        ...
        <footer class="editor-actions"><div><a class="daca-button is-secondary" routerLink="/models">Zur Übersicht</a><span>{{ form.dirty ? 'Ungespeicherte Änderungen' : 'Alle Änderungen gespeichert' }}</span></div><div><button class="daca-button is-secondary" type="button" [disabled]="model()?.status !== 'draft' || saving()" (click)="submitForReview()">Zur Domänenfreigabe einreichen</button>@if(model()?.status === 'review_pending' && model()?.reviewId && canDecideReview()){<button class="daca-button" type="button" [disabled]="saving()" (click)="acceptReview()">Annehmen und publizieren</button><button class="daca-button is-secondary" type="button" [disabled]="saving()" (click)="rejectReview()">Rückweisen</button>}@if(model()?.status === 'published'){<button class="daca-button is-secondary" type="button" [disabled]="!canPublishCurrentModel() || saving()" (click)="retire()">Modell stilllegen</button>}<button class="daca-button" type="submit" [disabled]="form.invalid || saving() || !identity.canEditModels() || model()?.status === 'retired' || model()?.status === 'review_pending'">{{ saving() ? 'Wird gespeichert …' : 'Als Entwurf speichern' }}</button></div></footer>
...
export class LogicalModelEditorComponent {
  ...
  readonly model = signal<LogicalModel | null>(null); readonly concepts = signal<readonly I14yConcept[]>([]); readonly loading = signal(false); readonly saving = signal(false); readonly exporting = signal<LogicalModelExportRepresentation | null>(null); readonly error = signal<string | null>(null); readonly notice = signal<string | null>(null); readonly etag = signal('');
  ...
  readonly form = this.fb.group({
    dataset: this.fb.nonNullable.group({ titleDe: ['', Validators.required], descriptionDe: ['', Validators.required], titleFr: '', descriptionFr: '', titleIt: '', descriptionIt: '', titleEn: '', descriptionEn: '', titleRm: '', descriptionRm: '', identifiers: ['', Validators.required], department: ['vbs', Validators.required], office: ['vbs-armasuisse', Validators.required], division: 'vbs-armasuisse-immobilien', dataDomainId: ['', Validators.required], dataOwnerId: ['', Validators.required], deputyDataOwnerId: '', creatorType: this.fb.nonNullable.control<CreatorType>('application'), creatorName: ['', Validators.required], creatorIdentifier: '', creatorPersonName: '', classification: this.fb.nonNullable.control<DataClassification>('unclassified'), dateCreated: [new Date().toISOString().slice(0,10), Validators.required] }),
  ...
  createField(order: number, field?: LogicalField, entity?: LogicalEntity, entityName = 'Entitaet_1') {
    ...
    return this.fb.group({
      id:field?.rootId??'',entityId:entity?.rootId??'',entityName:[entity?.name??field?.entityName??entityName,Validators.required],entityBusinessObject:entity?.businessObject??field?.businessObject??'',entityBusinessObjectVersionId:[entity?.businessObjectVersionId??'',Validators.required],entityComment:entity?.comment??'',entityOrder:entity?.order??order,name:[field?.name??'',Validators.required],businessObject:field?.businessObject??'',businessObjectVersionId:[field?.businessObjectVersionId??'',Validators.required],dataType:[field?.dataType??'xsd:string',Validators.required],length:field?.length??null as number|null,precision:field?.precision??null as number|null,decimalPlaces:field?.decimalPlaces??null as number|null,order:field?.order??order,nullable:field?.nullable??true,minCount:field?.minCount??0,maxCount:field?.maxCount??1 as number|null,classification:this.fb.nonNullable.control<DataClassification>(field?.classification??'unclassified'),sourceSystem:field?.sourceSystem??'',shortDescription:[field?.shortDescription??'',Validators.required],comment:field?.comment??'',
  ...
  save(): void { if (this.form.invalid || this.saving()) { this.form.markAllAsTouched(); return; } this.saving.set(true); this.error.set(null); this.notice.set(null); const current = this.model(); const value = this.value(); const request = current ? this.api.updateLogicalModel(current.id, current.versionId, value, this.etag()) : this.api.createLogicalModel(value); request.pipe(finalize(() => this.saving.set(false))).subscribe({ next: ({ body, etag }) => { this.model.set(body); this.etag.set(etag); this.form.markAsPristine(); this.notice.set('Der unabhängige Modellentwurf wurde versioniert gespeichert.'); if (!current) void this.router.navigate(['/models', body.id]); }, error: (error: Error) => this.error.set(error.message) }); }
