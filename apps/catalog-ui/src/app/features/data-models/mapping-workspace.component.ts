...
@Component({
  ...
  imports: [RouterLink, DataModelWorkspaceNavComponent, DatasetSummaryComponent, LogicalModelEditorComponent, MappingGraphComponent, MappingMatrixComponent, MappingDriftComponent, MappingEditorDialogComponent, MappingInspectorComponent, MappingInconsistenciesComponent],
  ...
  template: `
    ...
    @if(!loading()&&facade.model();as model){
      ...
      @if(!fallbackActive()){<daca-mapping-inconsistencies [modelId]="model.id" [refreshKey]="issueRefreshKey()" />}
      ...
      <section class="mapping-shell daca-card">
        ...
        <div class="mapping-body" [class.is-drift]="facade.view()==='drift'">
          <main>@switch(facade.view()){@case('graph'){<daca-mapping-graph [canManage]="canManageSelectedModel()" (fieldActivated)="focusSelectedFieldInput()" (disconnect)="disconnectFieldLink($event.mappingId,$event.columnId)" (deleteField)="deleteLogicalField($event)"/>}@case('table'){<daca-mapping-matrix/>}@case('drift'){<daca-mapping-drift/>}}</main>
  ...
  `, `
    .mapping-body{grid-template-columns:minmax(570px,1fr) minmax(340px,380px)}
    @media(max-width:999px){.mapping-body{grid-template-columns:1fr}.mapping-side{position:static;max-height:none;overflow:visible}}
  `],
})
export class MappingWorkspaceComponent {
  ...
  focusSelectedFieldInput():void{setTimeout(()=>this.host.nativeElement.querySelector<HTMLInputElement>('.mapping-side .field-panel-form input[formcontrolname="entityName"]')?.focus(),0);}
