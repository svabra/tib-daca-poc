...
@Component({
  selector: 'didaca-product-quality-wizard',
  standalone: true,
  ...
  template: `
    ...
    @if (state(); as current) {
      ...
      <div class="quality-layout">
        ...
        <form class="didaca-card quality-workspace" (submit)="$event.preventDefault(); save()">
          @if (step() === 1) { <header><p class="didaca-eyebrow">Schritt 1</p><h2>Technische Metadaten</h2><p>DAAIF hat Schema und Datentypen automatisch geliefert.</p></header><div class="quality-fields">@for (field of current.fields; track field.id) { <article><code>{{ field.name }}</code><strong>{{ field.dataType }}</strong><span>{{ field.nullable ? 'nullable' : 'required' }}</span><label><input type="checkbox" [checked]="field.keyField" (change)="setKey(field, $any($event.target).checked)"> Fachliches Kernfeld</label></article> }</div> }
          @if (step() === 2) { <header><p class="didaca-eyebrow">Schritt 2</p><h2>Fachliche Metadaten</h2></header><div class="quality-form-grid"><label>Titel<input [value]="title()" (input)="title.set($any($event.target).value)"></label><label>Fachgebiet<input [value]="domain()" (input)="domain.set($any($event.target).value)"></label><label class="is-wide">Beschreibung<textarea rows="4" [value]="description()" (input)="description.set($any($event.target).value)"></textarea></label><label>Klassifikation<select [value]="classification()" (change)="classification.set($any($event.target).value)"><option value="public">Öffentlich</option><option value="internal">Intern</option><option value="confidential">Vertraulich</option><option value="restricted">Eingeschränkt</option></select></label><label>Aktualisierung<input [value]="frequency()" (input)="frequency.set($any($event.target).value)"></label><label class="is-wide">Kontakt<input type="email" [value]="contactEmail()" (input)="contactEmail.set($any($event.target).value)"></label></div><div class="quality-field-descriptions"><h3>Attributbeschreibungen</h3>@for (field of current.fields; track field.id) { <label><code>{{ field.name }}</code><input [value]="field.businessDescription ?? ''" (input)="setDescription(field, $any($event.target).value)" placeholder="Fachliche Bedeutung beschreiben"></label> }</div> }
          @if (step() === 3) { <header><p class="didaca-eyebrow">Schritt 3</p><h2>DCAT-AP-CH kontrollieren</h2><p>DaCa bildet Produkt und REST-Service auf die Katalogbegriffe ab. Jede Zuordnung wird einzeln bestätigt.</p></header><div class="quality-dcat">@for (mapping of dcatMappings; track mapping.term) { <article><code>{{ mapping.term }}</code><strong>{{ dcatValue(mapping.term) }}</strong><label><input type="checkbox" [checked]="dcatConfirmed()[mapping.term]" (change)="setDcatConfirmation(mapping.term, $any($event.target).checked)"> OK</label></article> }</div><p class="didaca-alert">DCAT-AP-CH beschreibt den Katalogeintrag; die fachliche Bedeutung wird separat im nächsten Schritt verankert.</p> }
          @if (step() === 4) { <header><p class="didaca-eyebrow">Schritt 4 · Kanonische Ontologie</p><h2>Fachliche Bedeutung verankern</h2><p>DCAT beschreibt den Katalogeintrag. Die DaCa-Steuerontologie beschreibt, was Produkt und Kernfelder bedeuten.</p></header><label>Produktklasse<select [value]="productClassUri()" (change)="productClassUri.set($any($event.target).value)"><option value="">Bitte auswählen</option>@for (term of classTerms(); track term.id) { <option [value]="term.uri">{{ term.label }}</option> }</select></label><div class="quality-ontology-fields">@for (field of keyFields(current); track field.id) { <label><span><code>{{ field.name }}</code><small>Kernfeld</small></span><select [value]="fieldTerm(field.id)" (change)="setFieldTerm(field.id, $any($event.target).value)"><option value="">Noch nicht zugeordnet</option>@for (term of propertyTerms(); track term.id) { <option [value]="term.uri">{{ term.label }}</option> }</select></label> }</div><p class="didaca-alert"><strong>DaCa Canonical Tax Ontology · PoC</strong><br>Nur bestätigte Begriffe der aktiven Version zählen für Platinum. I14Y-Links werden nicht erfunden.</p> }
          @if (step() === 5) { <header><p class="didaca-eyebrow">Schritt 5 · KOBY Graphify</p><h2>Kontextgraph prüfen</h2><p>Deterministische PoC-Simulation ohne externen AI-Aufruf.</p></header><div class="quality-graph"><span>DAAIF</span><i>publiziert</i><strong>{{ title() }}</strong><i>gehört zu</i><span>{{ domain() }}</span><i>bereitgestellt über</i><span>REST</span></div><label class="quality-confirm"><input type="checkbox" [checked]="graphConfirmed()" (change)="graphConfirmed.set($any($event.target).checked)"><span><strong>Kontextgraph bestätigt</strong><small>Die dargestellten Beziehungen sind fachlich nachvollziehbar.</small></span></label> }

          @if (error()) { <p class="didaca-alert is-error">{{ error() }}</p> }
          @if (saved()) { <p class="didaca-alert">Gespeichert. Der Qualitätsstatus wurde serverseitig neu berechnet.</p> }
          <footer><button class="didaca-button is-secondary" type="button" [disabled]="step() === 1" (click)="step.set(step() - 1)">Zurück</button>@if (step() < 5) { <button class="didaca-button" type="button" (click)="step.set(step() + 1)">Weiter</button> } @else { <button class="didaca-button" type="submit">Prüfung speichern</button> }</footer>
        </form>
      </div>
    } @else { <p class="didaca-alert is-warning">Qualitätsdaten werden geladen…</p> }
  `,
})
